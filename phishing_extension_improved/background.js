...
chrome.webNavigation.onBeforeNavigate.addListener((details) => {
  // Only analyze top-level navigations to avoid spamming notifications for iframes/ads
  if (details.frameId !== 0 && details.frameId !== undefined) {
    return;
  }
  if (typeof details.parentFrameId !== 'undefined' && details.parentFrameId !== -1) {
    return;
  }
  if (!details.url || details.url.startsWith('chrome://') || details.url.startsWith('about:') || details.url.startsWith('edge://')) {
    return;
  }
...
function handleAnalysisResult(result, tabId, url, settings = {}, frameId = 0, navigationTabId = null) {
  ...
  if (result.security_checks && result.security_checks.whitelist && result.security_checks.whitelist.checked && result.security_checks.whitelist.status === 'safe') {
    score = 10;
    riskLevel = 'low';
  } else if (result.security_checks && result.security_checks.model && result.security_checks.model.checked && typeof result.security_checks.model.confidence === 'number') {
...
function notifyUser(tabId, riskLevel, url, score, customMessage = null, settings = {}, prediction = null) {
  ...
  const message = {
    ...
    url: url,
    score: modelScore,
    securityChecks: prediction && prediction.security_checks ? prediction.security_checks : null
  };
...
async function checkUrl(url) {
    try {
        ...
        if (cached) {
            ...
            // Derive score from cached result (same logic as fresh response)
            let score = null;
            if (result && result.security_checks && result.security_checks.whitelist && result.security_checks.whitelist.checked && result.security_checks.whitelist.status === 'safe') {
                score = 10;
            } else if (result && result.security_checks && result.security_checks.model && typeof result.security_checks.model.confidence === 'number') {
                score = result.security_checks.model.confidence;
            } else if (typeof result?.result !== 'undefined') {
                score = result.result;
            } else if (typeof result?.risk_score !== 'undefined') {
                score = result.risk_score;
            } else if (result && result.risk_scores && typeof result.risk_scores.combined !== 'undefined') {
                score = result.risk_scores.combined;
            } else if (result && result.risk_scores && typeof result.risk_scores.model !== 'undefined') {
                score = result.risk_scores.model;
            }
        ...
        const apiKeysData = await new Promise((resolve) => {
        ...
        const response = await fetch('http://localhost:5000/predict', {
            ...
            body: JSON.stringify({ 
                url: url,
                apiKeys: {
                    googleSafeBrowsing: apiKeysData.googleSafeBrowsing || null,
                    virusTotal: apiKeysData.virusTotal || null
                }
            })
        });
        ...
        // Derive score from various possible response formats
        let score = null;
        if (result && result.security_checks && result.security_checks.whitelist && result.security_checks.whitelist.checked && result.security_checks.whitelist.status === 'safe') {
            score = 10;
        } else if (result && result.security_checks && result.security_checks.model && typeof result.security_checks.model.confidence === 'number') {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'updateApiKeys') {
        // API keys are stored in chrome.storage.local by popup.js
        // This is just a confirmation message
        console.log('API keys updated:', request.apiKeys);
        sendResponse({ success: true });
        return true;
    }
    if (request.action === 'validateApiKey') {
        validateApiKey(request.service, request.key).then(result => {
            sendResponse(result);
        });
        return true;
    }
    ...
    try {
        if (service === 'gsb') {
            ...
            if (response.status === 200 || response.status === 204) {
                ...
                if (errorData.error && errorData.error.message && errorData.error.message.includes('API key')) {
                    return { success: false, error: 'Invalid API key format' };
                }
                return { success: true, message: 'Valid API key (test request accepted)' };
            } else {
            ...
            if (response.status === 200) {
                const data = await response.json();
                // VirusTotal returns 200 even for invalid keys, but with error message
                if (data.response_code === 0 && data.verbose_msg && data.verbose_msg.includes('not found')) {
                    // This is actually OK - it means the API key works, just no report for this URL
                    return { success: true, message: 'Valid API key' };
                } else if (data.response_code !== undefined) {
                    return { success: true, message: 'Valid API key' };
                } else {
                    return { success: false, error: 'Invalid API key' };
                }
            } else if (response.status === 204) {
                return { success: false, error: 'Rate limit exceeded or invalid API key' };
